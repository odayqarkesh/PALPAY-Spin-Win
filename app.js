...
let gameData = {
  playedIds: new Set(),
  prizes: {
    prize25: 11  // 11 فائزين فقط من فئة 25 شيكل
  }
};
...
// ===== تعريف القطاعات (تبقى كما هي بصرياً) =====
const segments = [
...
// ===== وظيفة بدء الدوران (اللوجيك الجديد لاختيار الجوائز) =====
function startSpin() {
  ...
  const winnableSegments = segments.filter(segment => {
    if (segment.name === '50 شيكل' || segment.name === 'مطره مي') {
      return false;
    }
    if (segment.name === '25 شيكل' && gameData.prizes.prize25 <= 0) {
      return false;
    }
    return true;
  });
  ...
  if (winnableSegments.length === 0) {
      showError('عذراً، لقد نفدت جميع الجوائز القيمة!');
